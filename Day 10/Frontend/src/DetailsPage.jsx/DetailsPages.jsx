...
function DetailsPages() {
    ...
    const[cin,setcin]=useState("2024-03-09");
    const[cout,setcout]=useState("2024-06-18");
    ...
    return (
        <>
            ...
            <div className="h-[90px] bg-[#ff6a25] w-full flex justify-evenly items-center">
                <div className='flex flex-col'>
                    <span className='text-white text-[0.7rem] font-semibold'>AREA, LANDMARK OR PROPERTY NAME</span>
                    <input className='h-[40px] rounded-[10px] mt-[5px] pl-[12px] font-bold' type='text' value='Oriental Hotel, Varkala'></input>
                </div>
                <div className='flex flex-col'>
                    <span className='text-white text-[0.7rem] font-semibold'>CHECK IN</span>
                    <input className='h-[40px] rounded-[10px] mt-[5px] pl-[12px] pr-[12px] font-bold'
                    onChange={(e) => setcin(e.target.value)}
                     value={cin} type='date'></input>
                </div>
                <div className='flex flex-col'>
                    <span className='text-white text-[0.7rem] font-semibold'>CHECK OUT</span>
                    <input className='h-[40px] rounded-[10px] mt-[5px] pl-[12px] pr-[12px] font-bold'
                    onChange={(e) => setcout(e.target.value)}
                     type='date' value={cout}></input>
                </div>
                <div className='flex flex-col'>
                    <span className='text-white text-[0.7rem] font-semibold'>GUEST COUNT</span>
                    <input className='h-[40px] rounded-[10px] mt-[5px] pl-[12px] font-bold' 
                    onChange={(e) => setgc(e.target.value)}
                    type='number' value={gc}></input>
                </div>
                <div className='flex items-center'>
                    <button className='bg-white pl-[10px] pr-[10px] rounded-[10px] 
                    font-bold h-[40px] text-blue-800'>Update Search</button>
                </div>
            </div>

            <div className="bg-white flex absolute top-[25%] left-[8%]">            {/* Component 1 */}
